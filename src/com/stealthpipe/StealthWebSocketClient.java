    ...
    private void checkClientUUIDPrefix(byte[] data) {
        ...
        if (dataAsString.startsWith(prefix)) {
            try {
                ...
                ServerNetworkIo listener = server.getNetworkIo();
    ...
    public void onError(Exception ex) {
        ...
        if (!ModState.isClientConnectingToStealthServer.get()) {
            ...
            server.getPlayerManager().broadcast(
                    Text.literal("[StealthPipe] Error occurred in tunnel. Please restart the world and create a new session.").formatted(Formatting.RED),
                    false
            );
        }
